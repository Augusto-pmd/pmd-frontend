...
export async function GET(request: Request) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[API EXPENSES ERROR]", errorText);
      return NextResponse.json(
        { error: "Error al obtener los gastos", message: errorText },
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.error("[API EXPENSES POST] Invalid JSON body:", bodyText);
      return NextResponse.json(
...
export async function PATCH(request: Request) {
  try {
    ...
    try {
      ...
      console.error("[API EXPENSES PATCH] Invalid JSON body:", bodyText);
      return NextResponse.json(
        { error: "Invalid JSON in request body" },
        { status: 400 }
      );
